#pragma once

#include <Decathect/Core.h>

// TODO: Move these to a precompiled header
#include <string>
#include <functional>

namespace Decathect{


	enum class EventType
	{
		None = 0,
		WindowClose, WindowResize, WindowFocus, WindowLostFocus, WindowMoved,
		AppTick, AppUpdate, AppRener, // Maybe wont use these
		KeyPressed, KeyReleased,
		MouseButtonPressed, MouseButtonReleased, MouseMoved, MouseScrolled
	};

	// Bitfield for event categorys
	enum EventCategory
	{
		None = 0,
		EventCategoryApplication = BIT(0),
		EventCategoryInput		 = BIT(1),
		EventCategoryKeyboard	 = BIT(2),
		EventCategoryMouse		 = BIT(3),
		EventCategoryMouseButton = BIT(4)
	};

#define EVENT_CLASS_TYPE(type) static EventType GetStaticType() { return EventType::type; }\
								virtual EventType GetEventType() const override { return GetStaticType(); }\
								virtual const char* GetName() const override { return #type; }

#define EVENT_CLASS_CATEGORY(category) virtual int GetCategoryFlags() const override { return category; }

	class DCTHCT_API Event
	{
		friend class EventDispatcher;
	public:
		virtual EventType GetEventType() const = 0;
		virtual const char* GetName() const = 0; // Debug only
		virtual int GetCategoryFlags() const = 0;
		virtual std::string ToString() const { return GetName(); } // Debug only

		inline bool IsInCategory(EventCategory cat)
		{
			return GetCategoryFlags() & cat; // Perform bit-wise and on the category param against this events category flags
		}
	protected:
		bool m_Handled = false; /// If an event has been handled
	};

	class EventDispatcher
	{
		// the type T could be any Event type (Resize, MousePressed, etc...)
		template<typename T>
		// function which returns a bool and takes in T Event type
		using EventFn = std::function<bool(T&)>;
	public:
		// Create an instance of the Event Dispatcher with an Event reference
		EventDispatcher(Event& e)
			: m_Event(e)
		{}

		template<typename T>
		bool Dispatch(EventFn<T> func)
		{
			if (m_Event.GetEventType() == T::GetStaticType())
			{
				m_Event.m_Handled = func(*(T)&m_Event);
				return true;
			}
			return false;
		}
	private:
		Event& m_Event;
	};

	// Used for our logging
	inline std::ostream& operator<<(std::ostream& os, const Event& e)
	{
		return os << e.ToString();
	}
}

